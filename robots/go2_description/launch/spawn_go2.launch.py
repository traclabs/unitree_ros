...
def generate_launch_description():
    ...
    launch_args = [
        ...
        DeclareLaunchArgument("y", default_value="0.0", description="y at which to spawn Spot."),
        DeclareLaunchArgument(
            "z", default_value="0.84", description="Height at which to spawn Spot."
        ),
        DeclareLaunchArgument(
            "roll", default_value="0.0", description="Roll at which to spawn Spot."
        ),
        DeclareLaunchArgument(
            "yaw", default_value="-0.5", description="Yaw at which to spawn Spot."
        ),
        DeclareLaunchArgument(
            "robot_up",
            default_value="7.0",
        ...
        DeclareLaunchArgument(
            "use_simulator", default_value="True", description="whether to use Gazebo Simulation."
        ),
        DeclareLaunchArgument(
    ...
    bridge_config_file = os.path.join(
        get_package_share_directory("champ_bringup"), "config", "go2_bridge.yaml"
    )
    ...
    ground_truth_node = Node(
        ...
        parameters=[
            {
                "robot_pose_topic": "/model/go2/pose",
                "fixed_frame": "world",
                "robot_frame": LaunchConfiguration("robot_base_link"),
            }
        ],
    )
    ...
    launch_quadruped_controller = IncludeLaunchDescription(
        PathJoinSubstitution(
            [FindPackageShare("champ_bringup"), "launch", "go2_quadruped_controller.launch.py"]
        )
    )
    ...
    return LaunchDescription(
        ...
        + [
            ...
            TimerAction(period=LaunchConfiguration("robot_up"), actions=[stand_up]),
            odom_republish_node,
        ]
    )
